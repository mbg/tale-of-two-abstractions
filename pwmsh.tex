...
We present an encoding of (imperative, class-based) objects in pure Haskell
supporting most conventional features of object-oriented languages,
including object classes, inheritance, subtype polymorphism, and dynamic
dispatch.
[what's object classes?  does class-based above express the same idea?]
[do we say "but not aliasing"?]
We show a correspondence between inheritance and monad
transformers by defining a state monad transformer for each class,
parametrised only over the class's own state.
[These sentence seems to say two things: "a new connection", and "how we
use it"].
The methods of a class are
monadic computations which run on top of a stack of monad transformers,
consisting of one layer for the class and each of its ancestors.
The well-known monad-transformer limitation that functions must be `lifted'
into the right
place within a monad stack is overcome using dynamic dispatch.
[... and this is appropriate for OO?]
\end{abstract}
...
\item An encoding of...
\item Is open
\item Circumvents the need for ``lifting''
\end{itemize}
